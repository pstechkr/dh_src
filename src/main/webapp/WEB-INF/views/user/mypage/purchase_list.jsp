...
<script type="text/javascript">$("nav#lnb ul li:nth-child(2)").addClass("active");</script>
<section id="mypage">
	<h2>구매내역</h2>
	<div class="state">${user_name}님이 지금까지 만나본 엘루비는 총 <span>${total_count}</span>건 입니다.</div>
	<section class="list_table">
		<div class="list_header">
			<button onclick="javascript:location.href='${pageContext.request.contextPath}/mypage/purchase'">최근 1개월</button>
			<button onclick="javascript:location.href='${pageContext.request.contextPath}/mypage/purchase?m=6'">최근 6개월</button>
			<button onclick="javascript:location.href='${pageContext.request.contextPath}/mypage/purchase?m=12'">1년</button>
			<span>* 최근 1년 이내의 구매 내역을 확인할 수 있습니다. </span>
		</div>
		<table summary="예약 및 구매완료된 상품을 확인할 수 있습니다.">
			...
			<c:forEach var="list" items="${ticketInfoList}" varStatus="status">
				<tr class="${list.schedule_seq} ${list.product}">
					...
					<td class="review">
						<c:choose>
						<c:when test="${list.point == null}">
							<button class="like point" onclick="javascript:point('${list.schedule_seq}','${list.product}','1');">완전대박</button>
							<button class="dislike point" onclick="javascript:point('${list.schedule_seq}','${list.product}','0');">별로에요</button>
						</c:when>
						<c:otherwise>
							<c:if test="${list.point == '1'}">
							<button class="like point">완전대박</button>
							</c:if>
							<c:if test="${list.point == '0'}">
							<button class="dislike point">별로에요</button>
							</c:if>
						</c:otherwise>
						</c:choose>
